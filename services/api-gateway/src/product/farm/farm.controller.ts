...
import { ApiTags, ApiOperation, ApiResponse, ApiBody, ApiParam, ApiQuery, ApiConsumes, ApiBadRequestResponse, ApiUnauthorizedResponse, ApiNotFoundResponse, ApiInternalServerErrorResponse } from '@nestjs/swagger';
...
export class FarmController {
    ...
    @Post('register')
    @ApiOperation({ summary: 'Register a new farm', description: 'Registers a new farm for the authenticated user.' })
    @ApiBody({ type: FarmRegistrationDto })
    @ApiResponse({ status: 201, description: 'Farm registered successfully', type: Farm })
    ...
    @Post("verify/:farmId")
    @ApiOperation({ summary: 'Verify a farm', description: 'Uploads verification files (CCCD and biometric video) for a farm.' })
    @ApiParam({ name: 'farmId', description: 'ID of the farm to verify' })
    @ApiConsumes('multipart/form-data')
    @ApiBody({ schema: { type: 'object', properties: { cccd: { type: 'string', format: 'binary' }, biometric_video: { type: 'string', format: 'binary' } } } })
    @ApiResponse({ status: 200, description: 'Farm verified successfully', type: Farm })
    @ApiBadRequestResponse({ description: 'Missing CCCD or biometric video, or verification failed' })
    @ApiUnauthorizedResponse({ description: 'Unauthorized' })
    @ApiInternalServerErrorResponse({ description: 'Internal server error' })
    @UseInterceptors(
    ...
    @Get("my/farm")
    @ApiOperation({ summary: 'Get my farm', description: 'Retrieves the farm associated with the authenticated user.' })
    @ApiResponse({ status: 200, description: 'Farm retrieved successfully', type: Farm })
    @ApiUnauthorizedResponse({ description: 'Unauthorized' })
    ...
    @Public()
    @Get("owner/:userId")
    @ApiOperation({ summary: 'Get farm by user ID', description: 'Retrieves the farm for a specific user.' })
    ...
    @Public()
    @Get("all")
    @ApiOperation({ summary: 'List all farms', description: 'Retrieves a paginated list of all farms.' })
    ...
    @Public()
    @Get("search")
    @ApiOperation({ summary: 'Search farms', description: 'Searches for farms with filters and pagination.' })
    ...
    @ApiQuery({ name: 'page', required: false, type: Number })
    @ApiQuery({ name: 'limit', required: false, type: Number })
    @ApiResponse({ status: 200, description: 'Paginated search result', type: [Farm] })
    @ApiInternalServerErrorResponse({ description: 'Internal server error' })
    ...
    @Public()
    @Get(":farmId")
    @ApiOperation({ summary: 'Get farm by ID', description: 'Retrieves a farm by its ID.' })
