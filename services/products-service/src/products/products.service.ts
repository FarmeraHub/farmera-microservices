...
export class ProductsService {
  ...
  async create(
    ...
    try {
      if (files.product_images && files.product_images.length > 0) {
        savedImageResults = await this.fileStorageService.saveFiles(
      ...
      if (files.product_videos && files.product_videos.length > 0) {
        savedVideoResults = await this.fileStorageService.saveFiles(
      ...
      if (
        createProductDto.subcategory_ids &&
        createProductDto.subcategory_ids.length > 0
      ) {
        const notFoundSubID: number[] = [];
        for (const subId of createProductDto.subcategory_ids) {
          const subcategory =
            await this.categoriesService.checkSubcategoryById(subId);
          if (!subcategory) {
        ...
        if (notFoundSubID.length > 0) {
          throw new NotFoundException(
            `Không tìm thấy các danh mục phụ với ${notFoundSubID.join(', ')}`,
          );
        } else {
          for (const subId of createProductDto.subcategory_ids) {
      ...
      this.logger.log(
        `(create) Transaction đã commit thành công cho sản phẩm ID: ${savedProduct.product_id}`,
      );

      return savedProduct;
    } catch (error) {
      // Bắt lỗi trong quá trình lưu file hoặc thao tác DB
  ...
  async deleteProduct(
    ...
    try {
      await queryRunner.manager.delete(ProductSubcategoryDetail, {
        product: { product_id: productId },
      });

      const deleteResult = await queryRunner.manager.delete(Product, {
  ...
  async updateProduct(
    ...
    const currentSubcategoryIds =
      product.productSubcategoryDetails
        ?.map((detail) => detail.subcategory?.subcategory_id)
        .filter((id): id is number => id !== undefined && id !== null) || [];

    this.logger.log(
      `(updateProduct) Product found: ${JSON.stringify(product, null, 2)}`,
    );
    this.logger.log(
      `(updateProduct) currentSubcategoryIds ${currentSubcategoryIds}`,
    );
    ...
    try {
      ...
      const finalImageUrlsToKeep =
        updateProductDto.image_urls !== undefined
          ? updateProductDto.image_urls
          : oldImageUrls;
      oldImageUrls.forEach((oldUrl) => {
        if (!finalImageUrlsToKeep.includes(oldUrl)) {
          urlsToDeletePhysically.push(oldUrl);
        }
      });
      const finalVideoUrlsToKeep =
        updateProductDto.video_urls !== undefined
          ? updateProductDto.video_urls
          : oldVideoUrls;
      oldVideoUrls.forEach((oldUrl) => {
        if (!finalVideoUrlsToKeep.includes(oldUrl)) {
          urlsToDeletePhysically.push(oldUrl);
        }
      });
      ...
      if (updateProductDto.product_name !== undefined)
        fieldsToUpdate.product_name = updateProductDto.product_name;
      if (updateProductDto.description !== undefined)
        fieldsToUpdate.description = updateProductDto.description;
      if (updateProductDto.price_per_unit !== undefined)
        fieldsToUpdate.price_per_unit = updateProductDto.price_per_unit;
      if (updateProductDto.unit !== undefined)
        fieldsToUpdate.unit = updateProductDto.unit;
      if (updateProductDto.stock_quantity !== undefined)
        fieldsToUpdate.stock_quantity = updateProductDto.stock_quantity;
      if (updateProductDto.weight !== undefined)
        fieldsToUpdate.weight = updateProductDto.weight;
      //check logic nếu đăng bán ...
      if (updateProductDto.status !== undefined)
        fieldsToUpdate.status = updateProductDto.status;
      // Gộp và gán URLs cuối cùng
      fieldsToUpdate.image_urls = [
        ...new Set([...finalImageUrlsToKeep, ...newImageUrlsFromFile]),
      ];
      fieldsToUpdate.video_urls = [
        ...new Set([...finalVideoUrlsToKeep, ...newVideoUrlsFromFile]),
      ];
      ...
      if (updateProductDto.subcategory_ids !== undefined) {
        ...
        const subcategoryIdsToDelete = currentSubcategoryIds.filter(
          (id) => !requestedSubcategoryIds.includes(id),
        );
        if (subcategoryIdsToDelete.length > 0) {
          await manager.delete(ProductSubcategoryDetail, {
            product: { product_id: productId },
            subcategory: { subcategory_id: In(subcategoryIdsToDelete) },
          });
        }
        // ... (logic thêm detail mới) ...
        const subcategoryIdsToAdd = requestedSubcategoryIds.filter(
          ...
          if (subcategoryIdsToAdd.length > 0) {
            ...
            for (const subId of subcategoryIdsToAdd) {
              ...
              if (subcategoryExists) {
                ...
                this.logger.warn(
                  `(updateProduct) Subcategory ID ${subId} không tồn tại.`,
                );
              }
            }
            if (notFoundSubcategoryIds.length > 0) {
              ...
              throw new NotFoundException(
                `Các subcategory với ID [${notFoundSubcategoryIds.join(', ')}] không tồn tại.`,
              );
            }
            ...
            if (newDetailsToAdd.length > 0) {
              this.logger.log(
                `(updateProduct) Saving ${newDetailsToAdd.length} new subcategory details...`,
              );
              ...
              this.logger.warn(
                `(updateProduct) No valid new subcategory details to add, although IDs were requested.`,
              );
            }
          }
        }
      ...
      await queryRunner.commitTransaction();
      this.logger.log(
        `(updateProduct) Transaction committed successfully for product ID: ${productId}.`,
      );
      ...
      return this.productsRepository.findOne({
        where: { product_id: productId },
        relations: [
          // Load lại các relations bạn muốn trả về
          'farm',
          'productSubcategoryDetails',
          'productSubcategoryDetails.subcategory',
      ...
      this.logger.error(
        `(updateProduct) Error during transaction: ${error.message}`,
        error.stack,
      );
      await queryRunner.rollbackTransaction();
  ...
  async searchAndFilterProducts(
  ...
  ): Promise<PaginationResult<ResponseProductDto>> {
    ...
    if (filters?.subcategory) {
      queryBuilder.andWhere('subcategory.subcategory_id = :subcategoryId', {
    ...
    if (filters?.farmId) {
      queryBuilder.andWhere('farm.farm_id = :farmId', {
    ...
    if (paginationOptions.all) {
      ...
      const transformedProducts = await this.transformProductsToDto(products);
      // For backwards compatibility when all=true, we still return pagination structure
      const meta = new PaginationMeta({
        paginationOptions: Object.assign(new PaginationOptions(), {
          ...paginationOptions,
          page: 1,
  ...
  private async transformProductsToDto(
    products: Product[],
    ...
    return Promise.all(
      products.map(async (product) => {
        ...
        const subcategoryDetails =
          await this.productSubcategoryDetailRepository.find({
  ...
  async getProductsByFarmId(
    farmId: string,
    paginationOptions?: PaginationOptions,
  ): Promise<PaginationResult<ResponseProductDto> | ResponseProductDto[]> {
    ...
    // If no pagination options provided, return all products
    if (!paginationOptions) {
      const products = await queryBuilder.getMany();
      return await this.transformProductsToDto(products);
    }
  ...
  async searchAndFillterProducts(
    ...
    // Convert legacy parameters to new pagination system
    const paginationOptionsData = Object.assign(new PaginationOptions(), {
      page,
      limit,
      sort_by: sortBy,
      order: sortOrder?.toUpperCase() as any,
      all: false,
    });
    ...
    const result = await this.searchAndFilterProducts(
  ...
  async getProductsByIds(ids: string[]): Promise<any[]> {
    ...
    const productEntityMap = new Map(
      productEntities.map((p) => [p.product_id, p]),
    );
    ...
    if (farmIdsFromProducts.length > 0) {
      const farmEntitiesFromService =
        await this.farmsService.findFarmsByIds(farmIdsFromProducts);
    ...
    const allSubcategoryDetails: ProductSubcategoryDetail[] =
      await this.categoriesService.findProductSubcategoryDetailsByProductIds(
        foundProductIds,
      );

    const subcategoryDetailsByProductId = new Map<
      number,
      ProductSubcategoryDetail[]
    >();
    ...
    for (const originalProductId of productIds) {
      ...
      const farmEntity = productEntity.farm
        ? farmsMap.get(productEntity.farm.farm_id)
        : undefined;
      const responseFarm: Farm | undefined = farmEntity
        ? farmEntity
        : undefined;

      const productSpecificSubcategories =
        subcategoryDetailsByProductId.get(productEntity.product_id) || [];
      const categories =
        this.categoriesService.mapProductSubcategoryDetailsToCategoryDtos(
          productSpecificSubcategories,
        );

      results.push(new Product());
    }
    this.logger.log(
      `(getProductsByIds) PRE-STRINGIFY: Found ${results.length} products.`,
    );
    try {
      this.logger.log(
        `(getProductsByIds) STRINGIFYING: results content: ${JSON.stringify(results, null, 2)}`,
      );
    } catch (e) {
      this.logger.error(
        `(getProductsByIds) ERROR DURING JSON.stringify: ${e.message}`,
      );
      // Log một phần nhỏ hơn, an toàn hơn để xem cấu trúc
      if (results && results.length > 0) {
        this.logger.log(
          `(getProductsByIds) First result keys: ${Object.keys(results[0]).join(', ')}`,
        );
      }
    }
    return results;
  ...
  async findProductById(
    ...
    if (options?.includeCategory) {
      if (!relationsToLoads.includes('productSubcategoryDetails')) {
        // Kiểm tra và thêm nếu chưa có
        relationsToLoads.push('productSubcategoryDetails');
      }
      if (!relationsToLoads.includes('productSubcategoryDetails.subcategory')) {
        // Kiểm tra và thêm nếu chưa có
    ...
    this.logger.log(
      `(findProductById) Đang tìm sản phẩm với ID: ${productId} và các quan hệ: ${relationsToLoads.join(', ')}`,
    );
    const product = await this.productsRepository.findOne({
      where: { product_id: productId },
    ...
    this.logger.log(
      `(findProductById) Đã tìm thấy sản phẩm với ID: ${productId}`,
    );
    this.logger.log(
      `(findProductById) Product details: ${JSON.stringify(product, null, 2)}`,
    );
    return product;
  }
  ...
  async getProductById(id: string): Promise<ResponseProductDto> {
    ...
    if (!product) {
      this.logger.warn(
        `(getProductById) Sản phẩm không tồn tại với ID: ${productId}`,
      );
    ...
    this.logger.log(
      `(getProductById) Đã lấy được ProductEntity ID ${product.product_id}, bắt đầu map...`,
    );
    ...
    this.logger.log(
      `(getProductById) Map thành công ProductEntity ID ${product.product_id} sang ResponseProductDto.`,
    );
    // Bạn có thể log DTO đã map để kiểm tra nếu cần (cẩn thận nếu DTO lớn)
    this.logger.debug(
      `(getProductById) DTO sau khi map: ${JSON.stringify(responseDto, null, 2)}`,
    );
  ...
  private toResponseDto(product: Product): ResponseProductDto {
    ...
    if (
      product.productSubcategoryDetails &&
      product.productSubcategoryDetails.length > 0
    ) {
    ...
    const responseProductDto: ResponseProductDto = new ResponseProductDto({
      ...
      price_per_unit:
        typeof product.price_per_unit === 'string'
  ...
  async findProductsByIds(
    ...
    this.logger.log(
      `(findProductsByIds) Đang tìm sản phẩm với các ID: ${JSON.stringify(productIds)} và relations: ${JSON.stringify(uniqueRelationsToLoad)}`,
    );
    ...
    if (products.length === 0) {
      this.logger.warn(
        `(findProductsByIds) Không tìm thấy sản phẩm nào cho các ID: ${JSON.stringify(productIds)}`,
      );
    } else {
      this.logger.log(
        `(findProductsByIds) Đã tìm thấy ${products.length} sản phẩm.`,
      );
      this.logger.log(
        `(findProductsByIds) Product details: ${JSON.stringify(products, null, 2)}`,
      );
    }
    return products;
  }
  ...
  async findProductsByFarmId(farmId: string): Promise<Product[]> {
    ...
    this.logger.log(
      `(findProductsByFarmId) Đang tìm sản phẩm cho farm ID: ${farmId}`,
    );
    const products = await this.productsRepository.find({
    ...
    if (products.length === 0) {
      ...
      this.logger.log(
        `(findProductsByFarmId) Đã tìm thấy ${products.length} sản phẩm cho farm ID: ${farmId}`,
      );
      this.logger.log(
        `(findProductsByFarmId) Product details: ${JSON.stringify(products, null, 2)}`,
      );
    }
    return products;
  }
