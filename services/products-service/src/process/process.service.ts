...
export class ProcessService {
  ...
  async createProcess(
    ...
    try {
      if (files.process_images && files.process_images.length > 0) {
        ...
        imageCidsResult = await this.pinataStorageService.uploadMutipleFiles(
      ...
      if (files.process_videos && files.process_videos.length > 0) {
        ...
        videoCidsResult = await this.pinataStorageService.uploadMutipleFiles(
          files.process_videos,
        );
      }
      ...
      const transaction =
        await this.blockchainService.addProcess(createdProcess);
  ...
  async getOnChainProcess(processId: number) {
    ...
    if (!process) {
      throw new BadRequestException(
        'Không tìm thấy quy trình sản xuất trên blockchain',
      );
    }
    return process;
  }
  ...
  async getOnChainProductProcessIds(productId: string) {
    const processIds =
      await this.blockchainService.getProductProcessIds(productId);
    if (!processIds) {
      throw new BadRequestException(
        'Không tìm thấy quy trình sản xuất trên blockchain',
      );
  ...
  async getVerifiedProcesses(productId: string) {
    const processes = await this.processRepository.findBy({
      productId: productId,
    });

    const verifiedProcesses = await Promise.all(
