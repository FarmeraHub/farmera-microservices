...
export class BiometricsService {
  ...
  constructor(private readonly httpService: HttpService,
    ...
    if (!apiKey) {
      throw new InternalServerErrorException('FPT API Key is missing in configuration.');
    }
    this.fptApiKey = apiKey;
    if (!idrUrl) {
      throw new InternalServerErrorException('FPT IDR URL is missing in configuration.');
    }
    this.fptIdrUrl = idrUrl;
    if (!livenessUrl) {
      throw new InternalServerErrorException('FPT Liveness URL is missing in configuration.');
    }
