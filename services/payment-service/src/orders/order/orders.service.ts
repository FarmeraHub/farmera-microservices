...
import {
  ...
  GhnPaymentTypeId,
  GhnRequiredNote,
} from 'src/delivery/dto/ghnn-create-delivery.dto';
...
import {
  PaymentMethod,
  PaymentStatus,
} from 'src/common/enums/payment/payment.enum';
...
export class OrdersService {
  ...
  async createOrder(orderRequest: OrderRequestDto): Promise<Order | Issue[]> {
    ...
    const orderInfoValidationPromise =
      this.businessValidationService.validateOrderInfo(orderRequest.order_info);
    ...
    const [orderInfoValidationResult, ...subOrdersValidationResults] =
      await Promise.all([
    ...
    if (paymentType && paymentType !== 'COD' && paymentType !== 'PAYOS') {
      allIssues.push({
        ...
        details: 'Unsupported payment method',
        user_id: orderRequest.order_info.user_id,
      });
    }

    this.logger.debug(
      `method payment: ${orderRequest.order_info.payment_type}`,
    );
    ...
    if (paymentType === 'COD') {
      this.logger.log(
        `Routing to COD payment for user: ${validOrderInfo?.user.id}`,
      );
      return await this.createOrderWithTransactionForCOD(
        validSubOrders,
        validOrderInfo!,
        orderRequest,
      );
      ...
      this.logger.log(
        `Routing to PAYOS payment for user: ${validOrderInfo?.user.id}`,
      );
      return await this.createOrderWithTransactionForPayOS(
        validSubOrders,
        validOrderInfo!,
        orderRequest,
      );
  ...
  async createOrderWithTransactionForCOD(
    validSubOrders: ShippingFeeDetails[],
    validOrderInfo: { user: User; address: Location },
    ...
    try {
      await this.updateProductQuantities(
        validSubOrders,
        UpdateProductQuantityOperation.DECREASE,
      );
      this.logger.log(
        `Successfully decreased product quantities before transaction`,
      );
    } catch (error) {
      this.logger.error(
        `Failed to decrease product quantities: ${error.message}`,
      );
      return [
        {
    ...
    this.logger.log(
      `Starting order creation transaction for user: ${validOrderInfo?.user.id}`,
    );
    const transactionalManager = queryRunner.manager;

    this.logger.debug(
      `Valid suborders: ${JSON.stringify(validSubOrders, null, 2)}`,
    );
    ...
    try {
      const totalAmount = validSubOrders.reduce((sum, subOrder) => {
        return (
          sum +
          subOrder.products.reduce((productSum, product) => {
            return (
              productSum + product.price_per_unit * product.requested_quantity
            );
          }, 0)
        );
      }, 0);

      const resultGHN: GhnCreatedOrderDataDto[] = await this.createOrderGHN(
        validSubOrders,
        validOrderInfo,
        true,
      );
      ...
      const shippingAmount = resultGHN.reduce(
        (sum, item) => sum + item.fee.main_service,
        0,
      );
      const discount = 0;
      ...
      for (let i = 0; i < validSubOrders.length; i++) {
        ...
        this.logger.log(
          `Processing suborder ${i + 1}/${validSubOrders.length} for farm: ${subOrderData.farm_name}`,
        );
      ...
      this.logger.error(
        `Error during order creation transaction: ${error.message}`,
        error.stack,
      );
      ...
      await this.updateProductQuantities(
        validSubOrders,
        UpdateProductQuantityOperation.INCREASE,
      );
      throw error;
    } finally {
  ...
  async createOrderWithTransactionForPayOS(
    validSubOrders: ShippingFeeDetails[],
    validOrderInfo: { user: User; address: Location },
    orderRequest: OrderRequestDto,
  ): Promise<Order | Issue[]> {
    ...
    try {
      ...
      this.logger.debug(
        `Paytos response: ${JSON.stringify(payosOrder, null, 2)}`,
      );
      ...
      this.logger.debug(
        `Payment response by PayOS: ${JSON.stringify(payosOrder.data, null, 2)}`,
      );
      const savePayment = await this.paymentService.create(
        {
      ...
      for (let i = 0; i < validSubOrders.length; i++) {
        ...
        this.logger.debug(
          `Delivery 1111111: ${JSON.stringify(ghnOrderResult, null, 2)}`,
        );
        const delivery = await this.deliveryService.create(
          ghnOrderResult,
          createSubOrder,
      ...
      this.logger.log(
        `Order details for PAYOS: ${JSON.stringify(order, null, 2)}`,
      );
      return order;
    } catch (error) {
      await queryRunner.rollbackTransaction();
  ...
  async createOrderGHN(
    ...
    try {
      let codCost = 0;
      let paymentTypeId: GhnPaymentTypeId =
        GhnPaymentTypeId.NGUOI_NHAN_THANH_TOAN;
      ...
      const createGHN: CreateGhnOrderDto[] = validSubOrders.map((subOrder) => {
        ...
        return {
          ...
          from_ward_name: subOrder.ward,
          to_name: validOrderInfo!.address.name,
      ...
      this.logger.debug(
        `Valid suborders for GHN: ${JSON.stringify(createGHN, null, 2)}`,
      );
      ...
      const resultGHN: GhnCreatedOrderDataDto[] =
        await Promise.all(resultGHNPromises);
      return resultGHN;
    } catch (error) {
      this.logger.error(
        `Error creating GHN order: ${error.message}`,
        error.stack,
      );
      throw error;
    }
  ...
  /**
   * Get orders by user ID with optional status filtering and pagination
   */
  async getOrdersByUserId(
    ...
    queryBuilder.orderBy('order.created', 'DESC').skip(offset).take(limit);
  ...
  /**
   * Get order details by order ID and verify it belongs to the user
   */
  ...
  private async updateProductQuantities(
    ...
    try {
      ...
      this.logger.debug(
        `${operation} quantities for ${productUpdates.length} products`,
      );

      const updateResult =
        await this.productsGrpcClientService.updateProductsQuantity(
          productUpdates,
        );
      this.logger.debug(
        `kết quả giảm sản phẩm: ${JSON.stringify(updateResult, null, 2)}`,
      );
      updateResult.results.map((test) =>
        this.logger.log(
          `kết quả chi tiết giảm sản phẩm: ${JSON.stringify(test, null, 2)}`,
        ),
      );
      ...
      if (!updateResult.success) {
        ...
        throw new Error(
          `Không thể cập nhật số lượng sản phẩm: ${failedProducts.join(', ')}`,
        );
      }

      this.logger.log(
        `Successfully ${operation} quantities for ${productUpdates.length} products`,
      );
    } catch (error) {
      this.logger.error(
        `Error ${operation} product quantities: ${error.message}`,
      );
      throw error;
    }
  }
